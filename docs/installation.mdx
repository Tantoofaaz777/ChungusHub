...
and the presets, characters and backgrounds the app ships with. It is the same layout as a release
...
For a copy you can move to another installation, open **Settings → Backups** and export any
snapshot in History. On the other installation, choose **Import backup**, select that ZIP, then
restore the imported row. Portable backups contain your stories, settings and API keys, but not
your password or authorized-device list. Restoring still asks you to restart ChungusHub so the
database can be replaced safely.
